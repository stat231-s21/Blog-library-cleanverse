...
The above maps were meant to provide a bird's eye view of the issue. If I'm interested in state's success with vaccination, that means looking at underlying factors that could explain differential population inoculation. Specifically, I wanted to know whether the proportion of vaccines administered out of the total relieved by a state  appeared to be correlated with the overall statewide progress. 

To look at that question, I first created the above maps. They allowed me to visualize, in a very easy way, whether it was worth considering vaccination ratio as a possible cause of some state's advancements. They confirmed, in my mind, that it was. As you can see, states with very dark shading on the left-hand map tended to also have dark shading on the right-hand map. In simpler terms, this means that when a state has much of its adult population fully vaccinated, it also usually is efficient about using the doses that it's given. This is an interesting result, as it suggests much of the differential progress states are making in fighting COVID is due to their own medical progress, not due to the government misallocating shots. 

The other benefit of creating these maps was that it allowed me to look at regional patterns. I could see that the South, in addition to some Great Plains states, had very low levels on both maps. Now, those are historically Republican regions. That inspired me to go ahead with my political research. 
...
In this section, I also created the scatterplot. The aim here was simple. In some circumstances, the eye can trick us. I wanted to confirm that the correlation I thought I saw actually existed, rather than being a result of my own bias. 

As we can see though, there is a reasonably strong, positive correlation between the perfect of the 18+ population that is fully vaccinated and the ratio administered. It's true that we see some variability and some clustering, but I would say it's far from a random scatter. Indeed, further investigation confirmed that a linear model predicting vaccination progress from the administration ratio explains 46% of the variability in vaccinations on the statewide level. That's almost half!

Given this research, there's a clear motive to understanding what's causing these failures in administration, as we can then better combat them and move towards vaccination. 

## Political Map, Boxplot

This section was intended to investigate the link between a state's voting record in the 2020 presidential election and their ability to administer as many doses as they receive. In all cases, when I describe a state as Democratic, I mean that it "went blue" in the 2020 election. In simpler terms, I mean that the majority of the population voted for Joe Biden. Conversely, when I say that a state is Republican, I mean that the majority of the population voted for Donald Trump, the incumbent. 

I chose this election to focus on as it's the most recent. Additionally, Trump has changed the dynamics of the Republican party, so whether they voted to reelect him is the most relevant information about a state's modern political inclinations.  
...
ggplot(ratio_map, aes(x = long, y = lat, group = group, fill = ratio_overall)) +
  ...
  theme(legend.position="bottom")
```
The above map shows many interesting results. First, we can see what we already knew: areas in the the South and Great Plains had some of the lowest ratios. However, it allows us to catch a much wider pattern. In simple terms, there is more dark blue than dark red. Conversely, there is more light red than light blue. 
...
What could explain this trend? Well, there has been a lot of misinformation spread by conspiracies such as [Q-Anon](https://www.washingtonpost.com/technology/2021/03/11/with-trump-gone-qanon-groups-focus-fury-attacking-covid-vaccines/), which has an easier time slipping into Republican discourse, as the conspiracy is pro-Trump and more pro-Republican. These conspiracies tell people not to get the vaccine, for fear of being tracked by the government. That misinformation is possibly responsible for the wasted doses: people simply don't want the shots. 

That's not to say that there's no misinformation on the Democratic side, but it might be impacting the sides differently, with strong impact in Republican states. 
...
To visualize the above information in another way, we can look at the above boxplot. Visual color patterns are a good indicator of what's happening, but this is a simplified diagram, that allows us to compare empirical numbers. 

The pattern on the map is dramatically pronounced here. The median rate of vaccination administration for states that voted for Joe Biden in 2020 is around 0.825. The median level for states that voted for Donald Trump is around 0.755. That's almost a 10% difference (0.07, to be exact). 

To summarize, then: we have evidence that one of the major factors impacting the speed at which states can vaccinate their populations is their ability to use all of the vaccine doses they're given. One underlying factor there is whether people are actually willing to be inoculated. You can't use all your shots if much of your population doesn't want them. 

The final bit of analysis showed that one reason people don't want doses is possibly because of their political inclination. We can see on a statewide level, that likely had an effect. There's certainly a difference in medians when stratified by voting history. 

The one outlive is Georgia, which is notably historically Republican, and flipped this election. 
