...
```{r}
kv_vaccine_states <- read_csv("us-daily-covid-vaccine-doses-administeredpopstate.csv")
...
ggplot() +
 geom_polygon(data=usa_vaccine_states
            , aes(x = long, y = lat, group = group
                  , fill = cumulative_vaccinations)
            ,color = "white") +
 theme_void() +
 ...
 labs(fill = "Number of People Vaccinated"
  ...
  scale_fill_distiller(palette = "YlGnBu", direction = "horizantle"
                       , limits = c(0,30000000)) +
 #geom_text(data=usa_vaccine_states, aes(y=45, x=-100, label=Day)) +
 #transition_time(time = test, range=c(14,15))
 transition_states(state = Day, transition_length=1
                      , state_length = 2
                      , wrap = TRUE)
#anim_save() 
