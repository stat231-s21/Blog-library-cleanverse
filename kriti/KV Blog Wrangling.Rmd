...
```{r}
kv_vaccine_states <- read_csv("us-daily-covid-vaccine-doses-administered_final.csv")
...
animate(anim_map_us, nframes = 2*length(unique(usa_vaccine_states$Day))
        , renderer = gifski_renderer("usavaccines.gif"))
...
ggplot() +
 ...
 theme_void() +
 coord_fixed(ratio = 1.3) +
 labs(fill = "USA Daily Vaccinations as a Percent of Population"
      , title = "Daily Vaccinations as a Percent of Population"
      , subtitle = "{closest_state}") +
      #,  subtitle="{format(frame_time, '%m/%d/%y')}"
...
animate(anim_map_perc, nframes = 2*length(unique(usa_vaccine_states$Day))
        , renderer = gifski_renderer("florida2.gif"))
